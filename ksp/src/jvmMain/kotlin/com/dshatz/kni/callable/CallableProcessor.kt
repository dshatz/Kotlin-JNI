    ...
    private fun generateCnameFunction(
        ...
        val paramCodes = params.map {
            ...
            val nullified = if (it.typeInfo.needsIsNullParam()) {
                CodeBlock.of("(if (%N.%M()) null else %L)", "_${it.name}IsNull", TypeMatcher.Method.ToKBoolean, paramCode.code)
        ...
        val callUserCode = if (funLocation.parent is FunLocation.FunctionParent.Class) {
            ...
            val unpackInstance = instanceType.unpackCode(CodeBlock.of("%N", "instance").returnType(TypeMatcher.KLong))
        ...
        val f = builder
            ...
            .addParameters(
                params
                    ...
                    .map {
                        ParameterSpec.builder("_${it.name}IsNull", TypeMatcher.JBoolean).build()
                    }
            )
            .addCode(
                CodeBlock.builder()
        ...
        return functions.flatMap { f ->
            ...
            val funSpec = FunSpec.builder(funNames[f]!!)
                ...
                .apply {
                    ...
                    val isNullParams = f.parameters.filter { it.typeInfo.needsIsNullParam() }.map {
                        CodeBlock.of("%N == null", it.name).returnType(TypeMatcher.KBoolean)
                    }
            ...
            val externalSpec = FunSpec.builder(externalMember)
                ...
                .addParameters(f.parameters.filter {
                    ...
                    ParameterSpec("_${it.name}IsNull", TypeMatcher.KBoolean)
                })
                .build()
